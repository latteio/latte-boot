 ...
 * @since : 2025/9/19
 */
public class CaffeineOpaqueTokenTemplateCondition implements Condition {
  public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
    Environment env = context.getEnvironment();

    boolean useRedis = env.getProperty("application.redis.enabled", Boolean.class, false);
    ...
    return useAuth
        && useToken
        && (!useRedis || "map".equalsIgnoreCase(tokenStore));
  }
}
